...
// se define la posisción de la camara para 1a persona
std::shared_ptr<FirstPersonCamera> camera(new FirstPersonCamera());

// se definen las primitivas principales que componen a nuestro modelo 
Sphere sphereAnimacion(20, 20);
Cylinder cylinderAnimacion(20, 20, 0.5, 0.5);
Cylinder cylinderAnimacion2(20, 20, 0.5, 0.5);
Box box;

// se definen los shader que corresponden  a   pequeños miniprogramas que
// ejecutan  tareas de forma específica.
Shader shaderColor; //color
Shader shaderTexture; // textura
Shader shaderCubeTexture; // textura de cubo
Shader shaderMateriales; //materiales
Shader shaderDirectionLight; // luz de la dirección
Shader shaderPointLight; // point
Shader shaderSpotLight; //spot
Shader shaderLighting; // contiene todas las luces

// modelos obtenidos desde web
Model modelRock;
Model modelRail;
Model modelAirCraft;
Model arturito;
...
textureWaterID, textureCubeTexture, textureMetalID;
GLuint cubeTextureID;

// float para incrementar losvalores de las rotaciones
float rot1 = 0.0f, rot2 = 0.0, rot3 = 0.0, rot4 = 0.0, rot5 = 0.0;
bool saveFrame = false, availableSave = true;


// texturas para el skybox
 // para el mapero se usa un mapa
// para positivo y negativo de cada eje 
GLenum types[6] = {
...
// se obtiene la ruta absoluta para las caras de nuestro skybox
...
GLFWwindow * window;
// se utiliza para salir de la pantalla cuando presionamos ESC y es verdadera 
bool exitApp = false;
// se utilizan para marcar en consola las  posiciones del movimeinto del mousse
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	//inicializacion de cada uno de los shaders  agregando el fragmet shader y el vertex shader 
	// co cada una de sus rutas absolutas 
	...
	// se inicializa cad aprimitiva 
	sphereAnimacion.init();
	sphereAnimacion.setShader(&shaderLighting); // se setea con el shader
	sphereAnimacion.setColor(glm::vec3(0.3, 0.3, 1.0));

	cylinderAnimacion.init();// inicialización de la primitiva 
	...
	/* para usar cada modelo se deb ecargar el .obj con load 
	...
	*/
	...
	/*Para cada una de las texturas se abre con su respectiva extensión
	 se agrega la estructura FIBITMAP  y se carga
	  se agregan los wrappings  a la textura 2D,, se define 
...
void applicationLoop() {
	...
	*/
	...
	while (psi) {
		...
		//convención de denavit-harterberg para calcular la cinemática directa de un cuerpo rigido 
		// Articulacion 1
		// solo se renderiza la matrizL lo demás no se guarda porque solo es para darle dimensión.
		...
		if (saveFrame)
			ss << matToString(matrixL0) << "|"; // separa con pipes todas las matrices de rotación(hecha en eje z)
